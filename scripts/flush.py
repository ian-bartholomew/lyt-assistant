...
from config import COMPILER_DIR, DAILY_DIR, STATE_DIR, FLUSH_LOG
...
def maybe_trigger_compilation() -> None:
    ...
    try:
        env = os.environ.copy()
        env["UV_PROJECT_ENVIRONMENT"] = str(Path.home() / ".local" / "share" / "lyt-assistant" / ".venv")
        log_handle = open(str(STATE_DIR / "compile.log"), "a")
        _sp.Popen(cmd, stdout=log_handle, stderr=_sp.STDOUT, cwd=str(COMPILER_DIR), env=env, **kwargs)
    except Exception as e:
