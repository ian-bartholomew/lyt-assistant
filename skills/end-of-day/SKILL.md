...
Run the full end-of-day capture pipeline: pull the day's meetings, extract Slack learnings (FES support to Confluence, support + internal channels to `raw/`), then compile everything into the wiki.

## Purpose

Provide a single command that runs the complete daily wrap-up workflow with interactive gates at every step. Chains together meeting ingestion, three Slack-learning extractions, and the full compile pipeline so the user doesn't have to remember to run each one manually at end of day.
...
Step 4.5: Join ─────────────────────┘  Wait for Steps 1 and 2 background subagents
                                        to finish before running compile.

Step 5: Compile                       Ingest raw/ into wiki/ + validate + discover links
                                      (compile workflow — consumes Steps 3 & 4 output)

Step 6: Report                        Summary of full pipeline run, appended to wiki/_log.md
```

Why this shape:

- **Step 1 in parallel:** `meeting-ingest` writes to `meetings/` (not `raw/`), uses Zoom MCP (not Slack), and nothing downstream reads its output. Fully independent, runs as a background subagent and overlaps with everything else.
...
- **Step 5 after the join:** `compile` reads from `raw/`, so it must run after Steps 3 and 4. It does not strictly depend on Steps 1 or 2, but the join ensures the final report is complete and the user isn't left wondering whether a background subagent is still running.

## Prerequisites

Each sub-skill has its own MCP requirements:
...
Don't pre-flight-check all three. Let each step fail naturally if its MCP is unauthed — the sub-skill itself reports the auth gap clearly. The failure-handling flow below covers that case.
...
Track: number of new threads processed, output file path(s). These files become inputs for Step 5.
...
Track: number of new threads processed and their categories (decision / incident / knowledge / process-change / discussion), output file path(s). These files also feed Step 5.
...
Not yet run:
  ...
  - Step 5: Compile — run /compile (will pick up Step 3 output plus Step 4 if you run it)
```
...
- **/compile** — Full compilation pipeline (Step 5; itself chains `/ingest` → `/lint` → `/discover-links`)

## Summary

The end-of-day skill runs the full daily wrap-up: pull the day's Zoom meetings, extract three Slack channels' worth of learnings (FES support to Confluence, support and internal channels to `raw/`), then compile `raw/` into the wiki. It chains five skills in order so the day's Slack-derived notes get into the wiki the same day rather than waiting until the next compile. Use `/end-of-day` as the single command to wrap up the workday.
